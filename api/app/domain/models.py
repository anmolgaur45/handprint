...
class TripLog(BaseModel):
    ...
    origin: str = Field(min_length=1, max_length=256, description="Starting location name")
    destination: str = Field(min_length=1, max_length=256, description="Ending location name")
...
class UserStreak(BaseModel):
    ...
    current_streak: int = Field(
        default=0, ge=0, description="Current consecutive active periods"
    )
    longest_streak: int = Field(
        default=0, ge=0, description="Longest consecutive active periods"
    )
